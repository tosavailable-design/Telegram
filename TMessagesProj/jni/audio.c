...
RubberBandState _rbState = 0;
int16_t *_rbBuffer = NULL;
size_t _rbBufferCount = 0;
size_t _rbBufferCapacity = 0;
...
int initRecorder(const char *path, opus_int32 sampleRate) {
    ...
    int rbOptions = RubberBandOptionProcessRealTime |
                    RubberBandOptionPitchHighQuality |
                    RubberBandOptionFormantPreserved;
    _rbState = rubberband_new(coding_rate, 1, rbOptions, 1.0, 0.75);
    _rbBufferCapacity = 8192;
    _rbBuffer = malloc(_rbBufferCapacity * sizeof(int16_t));
    _rbBufferCount = 0;
...
int writeFrame(uint8_t *framePcmBytes, uint32_t frameByteCount, int end) {
    ...
    opus_int32 nb_samples_in = frameByteCount / 2;

    if (nb_samples_in > 0) {
        float *in_float = malloc(nb_samples_in * sizeof(float));
        int16_t *pcm_in = (int16_t *)framePcmBytes;
        for (int i = 0; i < nb_samples_in; i++) {
            in_float[i] = pcm_in[i] / 32768.0f;
        }
        const float *inputs[1] = { in_float };
        rubberband_process(_rbState, inputs, nb_samples_in, end);
        free(in_float);
    } else if (end) {
        rubberband_process(_rbState, NULL, 0, 1);
    }

    int available = rubberband_available(_rbState);
    if (available > 0) {
        if (_rbBufferCount + available > _rbBufferCapacity) {
            _rbBufferCapacity = _rbBufferCount + available + 8192;
            _rbBuffer = realloc(_rbBuffer, _rbBufferCapacity * sizeof(int16_t));
        }

        float *out_float = malloc(available * sizeof(float));
        float *outputs[1] = { out_float };
        rubberband_retrieve(_rbState, outputs, available);

        for (int i = 0; i < available; i++) {
            float val = out_float[i] * 32768.0f;
            if (val > 32767.0f) val = 32767.0f;
            else if (val < -32768.0f) val = -32768.0f;
            _rbBuffer[_rbBufferCount + i] = (int16_t)val;
        }
        _rbBufferCount += available;
        free(out_float);
    }

    while (_rbBufferCount >= frame_size || (end && _rbBufferCount > 0)) {
        opus_int32 nb_samples = (opus_int32)(_rbBufferCount < frame_size ? _rbBufferCount : frame_size);
        
        _packetId++;
        total_samples += nb_samples;
        op.e_o_s = (end && _rbBufferCount <= frame_size);

        int nbBytes = 0;
        uint8_t *paddedFrameBytes = (uint8_t *)_rbBuffer;
        int freePaddedFrameBytes = 0;

        if (nb_samples < cur_frame_size) {
            paddedFrameBytes = malloc(cur_frame_size * 2);
            freePaddedFrameBytes = 1;
            memcpy(paddedFrameBytes, _rbBuffer, nb_samples * 2);
        ...
        _rbBufferCount -= nb_samples;
        if (_rbBufferCount > 0) {
            memmove(_rbBuffer, _rbBuffer + nb_samples, _rbBufferCount * sizeof(int16_t));
        }
    }
